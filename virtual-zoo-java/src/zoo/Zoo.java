...
import facility.park.Park;
import facility.restaurant.Restaurant;
import facility.road.Road;
import facility.road.entrance.Entrance;
import facility.road.exit.Exit;
import habitat.air.AirHabitat;
import habitat.land.LandHabitat;
import habitat.water.WaterHabitat;
import java.util.*;
import renderable.cage.Cage;
...
public class Zoo {
  ...
  /**
    ...
    * Isi kebun binatang lama akan hilang, terbentuk kebun binatang baru
  ...
  /**
    * Mengisi kandang ke kebun binatang.
    * @param in kandang yang akan dimasukkan
    */
  public void addCage(Cage in) {
    ...
    if ((in.getRow(i) < 0) || (in.getCol(i) < 0) 
         || (in.getRow(i) >= row) || (in.getCol(i) >= col)) {
      valid = false;
    } else {
  ...
  /**
    ...
    * @param in binatang yang ingin ditambahkan
    */
  ...
  public void read(Scanner in) {
    ...
    for (int i = 0; i < r; i++) {
      for (int j = 0; j < c; j++) {
        ...
        switch (k) {
          case 'W' : cell[i][j] = new WaterHabitat(); 
            break;
          case 'A' : cell[i][j] = new AirHabitat(); 
            break;
          case 'L' : cell[i][j] = new LandHabitat(); 
            break;
          case '*' : cell[i][j] = new Park(); 
            break;
          case 'R' : cell[i][j] = new Restaurant(); 
            break;
          case '-' : cell[i][j] = new Road(); 
            break;
          case 'i' : cell[i][j] = new Entrance(); 
            break;
          case 'o' : cell[i][j] = new Exit(); 
            break;
          default :
            break;
        }
